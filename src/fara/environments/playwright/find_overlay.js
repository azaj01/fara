...
(() => {
  const ID = '__agento_find__';
  const HL_CLASS = '__agento_find_hl__';
  const HL_ACTIVE = '__agento_find_hl_active__';
  ...
  if (!document.getElementById(ID + '_style')) {
    ...
    style.textContent =
      ...
      + '@keyframes __agento_caret_blink{50%{opacity:0}}'
      + '#' + ID + ' input::placeholder{'
        + 'animation:__agento_caret_blink 1s step-end infinite}';
